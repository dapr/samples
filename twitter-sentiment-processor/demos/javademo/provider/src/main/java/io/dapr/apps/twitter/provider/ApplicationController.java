...
public class ApplicationController {
   ...
   public Mono<Void> tweet(@RequestBody Tweet tweet) {
      ...
      return daprClient.invokeMethod(SENTIMENT_PROCESSOR_APP, "sentiment", tweet, HttpExtension.POST, Sentiment.class)
            .map(sentiment -> AnalyzedTweet.builder().id(tweet.getId()).tweet(tweet).sentiment(sentiment).build())
            .flatMap(analizedTweet -> daprClient.saveState(STATE_STORE, analizedTweet.getId(), analizedTweet)
                  .then(daprClient.publishEvent(PUBSUB, PUBSUB_TOPIC, analizedTweet).thenReturn(analizedTweet)))
            .doOnSuccess(analizedTweet -> log.info(String.format("Tweet saved %s: %s", analizedTweet.getId(),
                  analizedTweet.getSentiment().getSentiment())))
            .doOnError(onError -> log.error(String.format("Tweet not saved %s : %s", tweet.getId(), tweet.getText()),
                  onError))
            .then();
   }
